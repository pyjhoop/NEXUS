...
<script
	src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.18/js/i18n/defaults-ko_KR.min.js"></script>

<link href="https://fonts.googleapis.com/css2?family=Lato&display=swap"
	rel="stylesheet">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/core_issue_mini.css">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/apex-charts.css">
<link rel="stylesheet"
...
<body>
	...
	<div class="content-wrapper">
		...
		<input type="hidden" name="ino" value="">
			...
			<div class="card">
				<h5 class="card-header" style="padding-bottom: 0px;">NEXUS
					Project</h5>
									...
									<th><c:choose>
											<c:when test="${state eq 'open' }">
												<button type="submit" name="state" value="open"
													class="btn btn-success btn-sm">진행 중</button>
											</c:when>
											...
											<c:otherwise>
												...
												<button type="submit" name="state" value="open"
													class="btn btn-outline-success btn-sm">진행 중</button>
											</c:otherwise>
										</c:choose></th>
									<th><c:choose>
											<c:when test="${state eq 'closed' }">
												<button type="submit" name="state" value="closed"
													class="btn btn-secondary btn-sm">종료</button>
											</c:when>
											<c:otherwise>
												<button type="submit" name="state" value="closed"
													class="btn btn-outline-secondary btn-sm">종료</button>
											</c:otherwise>
										</c:choose></th>
									<th><c:choose>
											<c:when test="${state eq 'all' }">
												<button type="submit" name="state" value="all"
													class="btn btn-primary btn-sm">전체</button>
											</c:when>
											<c:otherwise>
												<button type="submit" name="state" value="all"
													class="btn btn-outline-primary btn-sm">전체</button>
											</c:otherwise>
										</c:choose></th>
								...
								<th style="width: 28%;">
									<form action="issueShow.mini" method="get" align="center">
										<select onchange="this.form.submit()"
											class="form-select form-select-sm" name="label"
								...
								<tr>
									...
									<td colspan="3" style="width: 26%"><a
										href="issueDetail.mini?ino=${i.number}" class="textA"> <i
											class="fab fa-angular fa-lg text-danger me-3"></i> <strong>${i.title}</strong>
									</a></td>
									...
									<td><c:forEach items="${i.labels}" var="label">
											<c:choose>
									...
									<td style="width: 10%;">
										<ul
											...
											<li data-bs-toggle="tooltip" data-popup="tooltip-custom"
												...
												title="${i.user }"><img src="${i.profile }" alt=""
									...
									<td style="width: 13%;">
										<ul
											class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
											<c:forEach items="${i.assignees}" var="assignee"
												varStatus="loop">
												<li data-bs-toggle="tooltip" data-popup="tooltip-custom"
													data-bs-placement="top" class="avatar avatar-xs pull-up"
													title="${assignee}"><img
													src="${i.assigneeProfiles[loop.index]}" alt=""
													class="rounded-circle" /></li>
