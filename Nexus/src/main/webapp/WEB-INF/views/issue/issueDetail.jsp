...
<body>
   ...
   <c:set var="member" scope="request" value="${ memberList}"/>
            ...
            <div>
               <c:forEach var="assignee" items="${assignees}">
                  <ul id="${assignee.userName}" class="mb-1 list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                     <li class="avatar avatar-xs pull-up"><img src="${assignee.profile}" alt="" class="rounded-circle"></li> ${assignee.userName}
                     <i i class="bx bx-x-circle mt-1 ml-1 assClose"></i>
                  </ul>
                  <script>
            ...
            <c:forEach items="${labels}" var="label">
               <c:choose>
                  <c:when test="${label eq 'bug'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-danger">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:when test="${label eq 'enhancement'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-info">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:when test="${label eq 'duplicate'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-dark">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:when test="${label eq 'documentation'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-primary">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:when test="${label eq 'invalid'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-warning">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:when test="${label eq 'help wanted'}">
                     <span class="labelSpan mr-1" id="${label}"><span class="badge rounded-pill bg-label-success">${label}</span><i class="bx bx-x-circle ml-1 labelClose"></i></span>
                  </c:when>
                  <c:otherwise>
  ...
  for (let i = 0; i < assigneeDropdown.length; i++) {
   ...
   function selectLabel(){
      ...
      function selectMile() {
           ...
           // 이전에 선택된 마일스톤 제거
           $(".milestoneSpan").remove();
           
           // 선택된 마일스톤 추가
           var str = '<span class="milestoneSpan mr-1" id="' + milestoneNum + '"><span class="badge rounded-pill bg-label-primary">' + milestoneTitle + '</span><i class="bx bx-x-circle ml-1 milestoneClose"></i></span>';
           $(".test2").append(str);
         }
